...
| D09 | accepted | Use direct Spring JDBC, explicit PostGIS SQL, and Flyway as sole DDL owner; H2/JPA are not integration truth. | `[implemented]` PostgreSQL runtime, test-only H2 context smoke, and Flyway-owned PostGIS extension migration | `[verified]` clean build plus fresh/reused real-PostGIS migration history |
...
| E05 | D09, R04 | Fresh/reused PostGIS migration and schema inspection. | Flyway alone owns DDL and repeat startup preserves migration integrity. | `[verified]` V1 `enable postgis` succeeded; restart retained schema version 1 and reported no migration necessary. |
