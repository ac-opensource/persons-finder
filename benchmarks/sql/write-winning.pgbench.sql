\set operation_id random(1, 2147483646)
...
WHERE person_id = benchmark.deterministic_uuid(
    'person',
    700001 + mod(:sequence_value, 50000)
)
FOR UPDATE;
...
VALUES (
    benchmark.deterministic_uuid('write-winning-observation', :sequence_value),
    benchmark.deterministic_uuid('person', 700001 + mod(:sequence_value, 50000)),
    timestamptz '2025-02-01 00:00:00+00'
        + :sequence_value * interval '1 microsecond',
    timestamptz '2025-02-01 00:00:00+00'
        + :sequence_value * interval '1 microsecond',
    'CLIENT_UPDATE',
    benchmark.deterministic_uuid('write-winning-client', :sequence_value),
    ST_SetSRID(
        ST_MakePoint(
            -0.10 + mod(:sequence_value, 1000) * 0.000001,
            51.50 + mod(:sequence_value, 1000) * 0.000001
        ),
        4326
    )::geography
);
...
SET
    observation_id = benchmark.deterministic_uuid(
        'write-winning-observation',
        :sequence_value
    ),
    captured_at = timestamptz '2025-02-01 00:00:00+00'
        + :sequence_value * interval '1 microsecond',
